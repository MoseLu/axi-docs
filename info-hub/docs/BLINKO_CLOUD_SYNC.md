...
- 在线检索入口以云端 Blinko 为准，Hermes / info-hub 始终连云端。
- 本机可以关机，云服务器继续跑定时同步。
...
- Info-Hub 应用目录：`/root/.hermes/apps/info-hub`
...
```bash
cp /root/.hermes/apps/info-hub/deploy/linux/blinko-sync.env.example /etc/blinko-sync.env
...
```bash
install -m 0644 /root/.hermes/apps/info-hub/deploy/linux/blinko-mirror-sync.service /etc/systemd/system/blinko-mirror-sync.service
install -m 0644 /root/.hermes/apps/info-hub/deploy/linux/blinko-mirror-sync.timer /etc/systemd/system/blinko-mirror-sync.timer
...
```bash
/bin/bash /root/.hermes/apps/info-hub/deploy/linux/blinko-nutstore-sync.sh
```
