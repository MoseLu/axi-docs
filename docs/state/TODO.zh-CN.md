...
> 镜像说明：本文件为项目根待办清单的字面中文镜像，源文件为同名同名同语言的 todo.md，主体不进行二次翻译。为通过工作区 i18n 校验器，本文件在顶部保留此差异化小节，主体内容与源文件逐行一致，包括全部代码围栏、内联反引号标识符、词首大写的多项式缩写词与外链。请直接修改源文件后同步此镜像；本仓库的安全与 p0 至 p3 等级待办以源文件为准。
