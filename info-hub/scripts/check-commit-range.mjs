...
const from = normalizeFromSha(readFlagValue('--from') ?? process.env.INFO_HUB_COMMIT_FROM ?? '');
const to = readFlagValue('--to') ?? process.env.INFO_HUB_COMMIT_TO ?? process.env.GITHUB_SHA ?? 'HEAD';
