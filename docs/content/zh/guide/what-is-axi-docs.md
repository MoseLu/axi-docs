...
文档来源由 `app/src/config/documentSources.ts` 注册。Guide 内容位于 `docs/content/{locale}/guide`，技能与工作区内容则来自各自的本地仓库。服务端索引这些来源，前端通过统一接口加载目录、正文、搜索结果和关系数据。

## 适合从哪里开始

第一次使用时先阅读[快速开始](/zh/guide/getting-started)，再根据目标进入技能库或工作区。需要维护内容时，从[文档来源](/zh/guide/document-sources)、[Markdown 写作](/zh/guide/markdown)和 [Frontmatter](/zh/guide/frontmatter)开始。
